...
class BusinessDataService {
    ...
    // ==================== 图表缓存 (chart_data_cache) ====================

    async setChartCache(type, dateRange, data) {
        const cache = {
            id: `${this.getCurrentUserId()}_${type}_${dateRange.start}_${dateRange.end}`,
            type,
            dateRange,
            data,
            userId: this.getCurrentUserId(),
        ...
        return indexedDBService.add('chart_data_cache', cache)
    }

    async getChartCache(type, dateRange) {
        const cacheId = `${this.getCurrentUserId()}_${type}_${dateRange.start}_${dateRange.end}`
        const cache = await indexedDBService.get('chart_data_cache', cacheId)
    ...
    async clearExpiredCache() {
        const now = Date.now()
        const caches = await indexedDBService.getAll('chart_data_cache')
        const userId = this.getCurrentUserId()

        for (const cache of caches) {
            if (cache.userId === userId && cache.expireTime <= now) {
                await indexedDBService.delete('chart_data_cache', cache.id)
            }
        }
    }
    ...
    async setSyncStatus(type, status) {
        const syncRecord = {
            id: `${this.getCurrentUserId()}_${type}`,
            userId: this.getCurrentUserId(),
            lastSync: new Date().toISOString(),
            status
        }
        return indexedDBService.update('sync_status', syncRecord)
    }

    async getSyncStatus(type) {
        const syncId = `${this.getCurrentUserId()}_${type}`
        return indexedDBService.get('sync_status', syncId)
    }

    async getPendingSyncData() {
        const [dailyRecords, incomeRecords, expenseRecords] = await Promise.all([
            indexedDBService.query('daily_records', 'syncStatus', 'pending'),
            indexedDBService.query('income_records', 'syncStatus', 'pending'),
            indexedDBService.query('expense_records', 'syncStatus', 'pending')
        ])

        const userId = this.getCurrentUserId()

        return {
            dailyRecords: dailyRecords.filter(r => r.userId === userId),
            incomeRecords: incomeRecords.filter(r => r.userId === userId),
            expenseRecords: expenseRecords.filter(r => r.userId === userId)
        }
    }

    async markAsSynced(ids, type) {
        for (const id of ids) {
            const record = await indexedDBService.get(type, id)
            if (record && record.userId === this.getCurrentUserId()) {
                record.syncStatus = 'synced'
                await indexedDBService.update(type, record)
            }
        }
    }
